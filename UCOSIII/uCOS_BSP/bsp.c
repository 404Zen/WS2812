...
#define  DWT_CR      *(CPU_REG32 *)0xE0001000           //DWT控制寄存器
#define  DWT_CYCCNT  *(CPU_REG32 *)0xE0001004           //DWT时钟计数寄存器
#define  DEM_CR      *(CPU_REG32 *)0xE000EDFC           //DEMCR寄存器
...
*                                            BSP_CPU_ClkFreq()
*
* Description : Read CPU registers to determine the CPU clock frequency of the chip.获取CPU时钟
*
* Argument(s) : none.
*
* Return(s)   : The CPU clock frequency, in Hz.	HCLK
*
* Caller(s)   : Application.
*
* Note(s)     : none.
...
void  CPU_TS_TmrInit (void)
{
	...
	DWT_CYCCNT      = (CPU_INT32U)0u;							//初始化CYCCNT寄存器
	DWT_CR         |= (CPU_INT32U)DWT_CR_CYCCNTENA;				//开启CYCCNT

	
	CPU_TS_TmrFreqSet((CPU_TS_TMR_FREQ)cpu_clk_freq_hz);
...
CPU_TS_TMR  CPU_TS_TmrRd (void)
{
	...
	ts_tmr_cnts = (CPU_TS_TMR)DWT_CYCCNT;
...
CPU_INT64U	CPU_TS32_to_uSec(CPU_TS64 ts_cnts)
{
	CPU_INT64U	ts_us;
	CPU_INT64U	fclk_freq;
