...
struct os_tcb {
    CPU_STK             *StkPtr;                            /* Pointer to current top of stack 指向当前任务栈顶                        */

    void                *ExtPtr;                            /* Pointer to user definable data for TCB extension 指向用户可定义的数据区      */

    CPU_STK             *StkLimitPtr;                       /* Pointer used to set stack 'watermark' limit 可以指向任务堆栈中的某个位置           */

    OS_TCB              *NextPtr;                           /* Pointer to next     TCB in the TCB list    双向链表            */
    OS_TCB              *PrevPtr;                           /* Pointer to previous TCB in the TCB list                */

    OS_TCB              *TickNextPtr;                       /* 把正在延时或者在指定时间内等待某个时间的任务的OS_TCB构成双向链表 */
    OS_TCB              *TickPrevPtr;                       /**/
    ...
    CPU_STK             *StkBasePtr;                        /* Pointer to base address of stack   任务堆栈基地址                    */

#if defined(OS_CFG_TLS_TBL_SIZE) && (OS_CFG_TLS_TBL_SIZE > 0u)
    OS_TLS               TLS_Tbl[OS_CFG_TLS_TBL_SIZE];
#endif

    OS_TASK_PTR          TaskEntryAddr;                     /* Pointer to task entry point address  任务代码入口地址                  */
    void                *TaskEntryArg;                      /* Argument passed to task when it was created  传递给任务的参数          */

    OS_PEND_DATA        *PendDataTblPtr;                    /* Pointer to list containing objects pended on  指向一个表，包含有任务等待时间的所有时间对象的信息         */
    OS_STATE             PendOn;                            /* Indicates what task is pending on   任务正在等待的事件的类型                   */
    OS_STATUS            PendStatus;                        /* Pend status           任务等待的结果                                 */

    OS_STATE             TaskState;                         /* See OS_TASK_STATE_xxx   任务当前的状态                               */
    ...
    CPU_TS               TS;                                /* Timestamp     存储时间发生的时间戳                                         */
